...
def avg_fc_gen(dfdata, df_ctype,
    ...
    for pop in tar_cell_pops:
        if cavg.shape[1] == 1:
            ...
            cells = classtype_sub[classtype_sub.iloc[
                :, class_col] != pop].index.tolist()
...
def preprocess_and_normalize(df_raw_expr, keep_location_data=False):
    ...
    df_norm = preprocessing.robust_scale(
        df_raw_expr, quantile_range=(1, 99), copy=True)
    df_norm = pd.DataFrame(
        df_norm, index=df_raw_expr.index, columns=df_raw_expr.columns)
...
def dimention_reduction(df_norm):
    ...
    umap_embedding = umap.UMAP(
        n_components=5, n_neighbors=15, min_dist=0.01, metric='correlation')
...
def plot_clustering(df_low_dim, df_labels):
    ...
    for cluster in sorted(df_labels.unique()):
        ...
        plt.scatter(df_low_dim[cells_idx, 0], df_low_dim[
                    cells_idx, 1], s=0.01, label=cluster)
...
def plot_expr_on_2D(df_2d, df_raw_expr):
    ...
    for col in df_raw_expr.columns:
        subplot = axes[axes_idx].scatter(df_2d[:, 0], df_2d[:, 1], c=df_raw_expr[
                                         col].values, s=0.01, cmap='bwr', label=col)
        axes[axes_idx].legend(frameon=False)
        fig.colorbar(subplot, ax=[axes[axes_idx], axes[
                     axes_idx]], pad=-0.05, extend='both', format='%.1f')
        axes_idx += 1
...
def get_top_markers(df_raw_expr, df_labels):
    ...
    """
    ctype = pd.DataFrame(
        df_labels.values, index=df_raw_expr.index, columns=['cluster'])
    ctype.to_csv('Cluster assignments.csv')
    _, df_fc = avg_fc_gen(df_raw_expr, ctype)
    df_report = pd.DataFrame(index=df_fc.columns, columns=[
                             'Top 5 markers', 'Markers exp'])
    for x in df_report.index:
        ...
        df_report.loc[x, 'Markers exp'] = ', '.join(
            [str(np.round(x, 2)) for x in top_5_markers_exp])
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Cy-cif segmented psudeo single-cell data Cell State Calling script. ')
    parser.add_argument('file_path', nargs='?', default=file_path,
                        help='Target data matrix of samples by features. Take segmented cycif data and output clustering figures, expression figures and top markers. Will use example file in /input folder by default.')
    parser.add_argument('-algo', metavar='Clustering algorithm', nargs='?', default=algorithm,
                        help="Clustering algorithm, select from ['KMeans','HDBSCAN']. KMeans by default.")
    parser.add_argument('-mcs', metavar='Minimal cluster size', nargs='?', default=mix_cluster_size,
                        type=int, help='Minimal cluster size for HDBSCAN clustering. 15 by default.')
    parser.add_argument('-nc', metavar='Number of clusters', nargs='?', default=n_clusters,
                        type=int, help='Number of clusters for KMeans clustering. 15 by default.')
    ...
    if len(cm_genes) < len(cfg_genes):
        print('Dropped genes in config file only: {}'.format(
            ', '.join(genes_in_cfg_only)))
        genes_in_cfg_only = [x for x in cfg_genes if x not in df_raw.columns]
    elif len(cm_genes) < df_raw.shape[1]:
        genes_in_input_only = [
            x for x in df_raw.columns if x not in cfg_genes]
        print('Dropped genes in input file only: {}'.format(
            ', '.join(genes_in_input_only)))
